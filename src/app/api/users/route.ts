...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userId) {
      ...
      return NextResponse.json({ id: String(user.uuid), name: user.name, email: user.email, avatar: user.avatar, role: user.role, dormId: user.dormId });
    }
    ...
    const viewer = await db.collection('users').findOne({ uuid: viewerId });
    if (!viewer || !(viewer as any).dormId) {
      return NextResponse.json([]);
    }

    const users = await db.collection('users').find({ dormId: (viewer as any).dormId }).toArray();
    return NextResponse.json(users.map(u => ({ id: String(u.uuid), name: u.name, email: u.email, avatar: u.avatar, role: u.role, dormId: u.dormId })));
  } catch (error) {
